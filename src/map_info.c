...
/*   Updated: 2025/03/04 00:29:07 by abonifac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_walls_horiz(t_game *game)
{
	...
	height = (game->map.height / TILE_S) - 2;
	y = 0;
...
void	map_walls_vert(t_game *game)
{
	...
	width = (game->map.width / TILE_S) - 1;
	x = 0;
	while (game->map.map[x] && x < (game->map.height / TILE_S) - 1)
	{
...
{
	if (game->map.width < 3 * TILE_S || game->map.height < 4 * TILE_S)
	{
...
{
	if (game->map.width == (game->map.height - TILE_S))
	{
...
void	check_invalid_char(t_game *game)
{
	...
	y = 0;
	x = 0;
	while ((game->map.height / TILE_S) - 1 > y)
	{
		x = 0;
